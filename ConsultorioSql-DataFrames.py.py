...
def realizar_cita():
    try:
        ...
        valid_patient_ids = {paciente[0] for paciente in pacientes}

        while True:
            clave_paciente = input("Ingrese la clave del paciente o escriba 0 para cancelar: ")
            if clave_paciente == "0":
            ...
            try:
                ...
                if clave_paciente == 0:
                    print("Saliendo del área de realizar citas...")
                    return menu_citas()
                if clave_paciente in valid_patient_ids:
                    break
                else:
                    print("No se encontró un paciente con esa clave. Por favor, intente de nuevo o escriba 0 para cancelar.")
            except ValueError:
                print("Clave inválida. Intente de nuevo o escriba 0 para cancelar.")
        ...
        while True:
            folio_cita_id = input("Ingrese el folio de la cita o escriba 0 para cancelar: ")
            if folio_cita_id == "0":
                print("Saliendo del área de realizar citas...")
                return menu_citas()
            try:
                folio_cita_id = int(folio_cita_id)
                if folio_cita_id == 0:
                    print("Saliendo del área de realizar citas...")
                    return menu_citas()
                ...
                if not cita:
                    print("No se encontró una cita pendiente con el folio indicado. Intente de nuevo o escriba 0 para cancelar.")
                    continue
                break
            except ValueError:
                print("Folio inválido. Intente de nuevo o escriba 0 para cancelar.")
        ...
        while True:
            try:
                peso = float(input("Ingrese el peso del paciente o escriba 0 para cancelar: "))
                if peso == 0:
                    print("Saliendo del área de realizar citas...")
                    return menu_citas()
                if peso < 0:
        ...
        while True:
            try:
                estatura = float(input("Ingrese la estatura del paciente o escriba 0 para cancelar: "))
                if estatura == 0:
                    print("Saliendo del área de realizar citas...")
                    return menu_citas()
                if estatura < 0:
                    raise ValueError("La estatura debe ser positiva.")
                break
            except ValueError as e:
                print(f"Error: {e}. Intente de nuevo o escriba 0 para cancelar.")
        ...
        while True:
            try:
                presion_sistolica = int(input("Ingrese la presión sistólica o escriba 0 para cancelar: "))
                if presion_sistolica == 0:
                    print("Saliendo del área de realizar citas...")
                    return menu_citas()
                if presion_sistolica < 0:
                    raise ValueError("La presión sistólica debe ser positiva.")
                break
            except ValueError as e:
                print(f"Error: {e}. Intente de nuevo o escriba 0 para cancelar.")
        ...
        while True:
            try:
                presion_diastolica = int(input("Ingrese la presión diastólica o escriba 0 para cancelar: "))
                if presion_diastolica == 0:
                    print("Saliendo del área de realizar citas...")
                    return menu_citas()
                if presion_diastolica < 0:
                    raise ValueError("La presión diastólica debe ser positiva.")
                break
            except ValueError as e:
                print(f"Error: {e}. Intente de nuevo o escriba 0 para cancelar.")
        ...
        while True:
            ...
            if len(diagnostico) > 200:
                print("Diagnóstico excede 200 caracteres. Intente de nuevo o escriba 0 para cancelar.")
                continue
            elif len(diagnostico) <=0:
                print("El diagnóstico no puede estar vacio. Intente de nuevo o escriba 0 para cancelar.")
                continue
            break
...
def cancelar_citas_por_fecha():
    try:
        ...
        while True:
            ...
            folios_disponibles = [cita[0] for cita in citas]
            ...
            while True:
                ...
                if folio_cita not in folios_disponibles:
                    print(f"El folio {folio_cita} no es válido para esta fecha. Por favor, ingrese un folio válido.")
                    continue
...
def buscar_por_clave_paciente():
    ...
    try:
        cursor.execute("SELECT clave_paciente, primer_apellido, segundo_apellido, nombre FROM pacientes")
        pacientes = cursor.fetchall()
        ...
        headers_pacientes = ["ID", "Primer Apellido", "Segundo Apellido", "Nombre"]
        print("Pacientes registrados:")
        print(tabulate(pacientes, headers=headers_pacientes, tablefmt="fancy_grid"))
        ...
        while True:
            ...
            if not expedientes:
                ...
                respuesta = input("¿Desea exportar solo los datos del registro del paciente? (S/N) o escriba 0 para cancelar: ").strip().upper()
                if respuesta == 'S':
                    exportar_datos_paciente_por_clave(paciente)
                elif respuesta == '0':
                    return
                continue

            print("Expediente clínico del paciente:")
            print(tabulate(expedientes, headers=["Folio Cita", "Fecha de Cita", "Hora de Llegada", "Turno", "Estado", "Peso", "Estatura", "Presión (Sis/Dias)", "Diagnóstico"], tablefmt="fancy_grid"))
...
def exportar_datos_paciente_por_clave(paciente, expedientes=None):
    ...
    if expedientes:
        ...
        df_final = df_paciente

    formato = input("Escriba '1' para exportar a CSV, '2' para exportar a Excel o '0' para cancelar la operación: ").strip()
    if formato == '1':
        df_final.to_csv('expediente_clinico_completo.csv', index=False)
        print("Datos exportados correctamente a 'expediente_clinico_completo.csv'.")
        return reportes_de_pacientes()
    elif formato == '2':
        df_final.to_excel('expediente_clinico_completo.xlsx', index=False)
        print("Datos exportados correctamente a 'expediente_clinico_completo.xlsx'.")
        return reportes_de_pacientes()
    elif formato == '0':
        print("Operación cancelada.")
...
def buscar_por_apellido_nombre():
    ...
    try:
        cursor.execute("SELECT clave_paciente, primer_apellido, COALESCE(segundo_apellido, '') as segundo_apellido, nombre FROM pacientes ORDER BY primer_apellido, segundo_apellido, nombre")
        pacientes = cursor.fetchall()
        ...
        while True:
            while True:
                primer_apellido = input("Ingrese el primer apellido del paciente o escriba 0 para cancelar: ").strip()
                if primer_apellido == '0':
                    return
            ...
            while True:
                nombre = input("Ingrese el nombre del paciente o escriba 0 para cancelar: ").strip()
                if nombre == '0':
                    return
            ...
            for resultado in resultados:
                ...
                if expedientes:
                    print("El paciente tiene expedientes clínicos disponibles.")
                    while True:
                        respuesta_ver = input("¿Desea ver el expediente clínico? (S/N) o escriba 0 para cancelar: ").strip().upper()
                        if respuesta_ver == 'S':
                            print("Expediente clínico del paciente:")
                            print(tabulate(expedientes, headers=["Folio Cita", "Fecha de Cita", "Hora de Llegada", "Turno", "Estado", "Peso", "Estatura", "Presión (Sis/Dias)", "Diagnóstico"], tablefmt="fancy_grid"))
                            exportar_decision(expedientes, resultado)
                            break
                        elif respuesta_ver == 'N':
                            break
                        elif respuesta_ver == '0':
                            return
                        else:
                            print("Por favor, responda con 'S' para sí, 'N' para no o escriba 0 para cancelar.")
                else:
                    print("El paciente no tiene expedientes clínicos.")
                    while True:
                        respuesta_exportar = input("¿Desea exportar los datos del registro del paciente? (S/N) o escriba 0 para cancelar: ").strip().upper()
                        if respuesta_exportar == 'S':
                            exportar_datos_registro([resultado])
                            break
                        elif respuesta_exportar == 'N':
                            break
                        elif respuesta_exportar == '0':
                            return
                        else:
                            print("Por favor, responda con 'S' para sí, 'N' para no o escriba 0 para cancelar.")
            break  
...
def exportar_decision(expedientes, registro):
    while True:
        respuesta = input("¿Desea exportar el expediente clínico junto con los datos del registro del paciente a CSV o Excel? (S/N) o escriba 0 para cancelar: ").strip().upper()
        if respuesta == 'S':
            exportar_datos(expedientes, registro)
            break
        elif respuesta == 'N':
            break
        elif respuesta == '0':
...
def exportar_datos(expedientes, registro):
    df_expedientes = pd.DataFrame(expedientes, columns=["Folio Cita", "Fecha de Cita", "Hora de Llegada", "Turno", "Estado", "Peso", "Estatura", "Presión Sistólica/Diastólica", "Diagnóstico"])
    df_registro = pd.DataFrame([registro], columns=["ID", "Primer Apellido", "Segundo Apellido", "Nombre", "Fecha Nac", "Edad", "Sexo"])
    df_final = pd.concat([df_registro] * len(df_expedientes), ignore_index=True).join(df_expedientes)
    
    while True:
        formato = input("Escriba '1' para exportar a CSV, '2' para exportar a Excel, o escriba 0 para cancelar: ").strip()
        if formato == '1':
            df_final.to_csv('datos_completos.csv', index=False)
            print("Datos exportados correctamente a 'datos_completos.csv'.")
            break
        elif formato == '2':
            df_final.to_excel('datos_completos.xlsx', index=False)
            print("Datos exportados correctamente a 'datos_completos.xlsx'.")
            break
        elif formato == '0':
            print("Operación cancelada.")
            return
        else:
            print("Opción inválida. Por favor, ingrese '1' para exportar a CSV, '2' para exportar a Excel, o escriba 0 para cancelar.")

def exportar_datos_registro(registro):
    df = pd.DataFrame(registro, columns=["ID", "Primer Apellido", "Segundo Apellido", "Nombre", "Fecha Nac", "Edad", "Sexo"])
    
    while True:
        formato = input("Escriba '1' para exportar a CSV, '2' para exportar a Excel, o escriba 0 para cancelar: ").strip()
        if formato == '1':
            df.to_csv('registro_paciente.csv', index=False)
            print("Datos del registro del paciente exportados correctamente a 'registro_paciente.csv'.")
            break
        elif formato == '2':
            df.to_excel('registro_paciente.xlsx', index=False)
            print("Datos del registro del paciente exportados correctamente a 'registro_paciente.xlsx'.")
            break
        elif formato == '0':
            print("Operación cancelada.")
            return
